...
main (int argc, char *argv[])
{
  ...
  NodeContainer terminals[2];
  for (int pod = 0; pod < 2; pod ++) {
    terminals[pod].Create (2);
  }

  NodeContainer csmaSwitch[2];
  for (int pod=0; pod < 2; pod++) {
    csmaSwitch[pod].Create(1);
  }
  ...
  NetDeviceContainer terminalDevices[2];
  NetDeviceContainer switchDevices[2];
  for (int pod = 0; pod < 2; pod++) {
      for (int i = 0; i < 2; i++)
      {
        NetDeviceContainer link = csma.Install (NodeContainer (terminals[pod].Get (i), csmaSwitch[pod]));
        terminalDevices[pod].Add (link.Get (0));
        switchDevices[pod].Add (link.Get (1));
      }

    // Create the switch netdevice, which will do the packet switching
    Ptr<Node> switchNode = csmaSwitch[pod].Get (0);
    OpenFlowSwitchHelper swtch;

    if (use_drop)
      {
        Ptr<ns3::ofi::DropController> controller = CreateObject<ns3::ofi::DropController> ();
        swtch.Install (switchNode, switchDevices[pod], controller);
      }
    else
      {
  ...
  p2p.install(csmaSwitch[0].Get(0), csmaSwitch[1].Get(0))


  // Add internet stack to the terminals
  InternetStackHelper internet;
  for (int pod=0; pod<2; pod++) {
    internet.Install (terminals[grp]);
  }
  ...
  for (int pod=0; pod<2; pod++) {
    ipv4.Assign (terminalDevices[grp]);
  }
  ...
  ApplicationContainer app = onoff.Install (terminals[0].Get (0));
  ...
  app = sink.Install (terminals[0].Get (1));
  ...
  app = onoff.Install (terminals[1].Get (1));
  app.Start (Seconds (1.1));
  app.Stop (Seconds (10.0));

  app = sink.Install (terminals[0].Get (0));
  app.Start (Seconds (0.0));
